...
import { ArrowLeft, Upload, Download, RefreshCw, SlidersHorizontal, Loader2, AlertCircle } from 'lucide-react';
...
import type { AudioBalancerProps } from './types';
...
export const AudioBalancer = ({ onBack }: AudioBalancerProps) => {
  ...
  const handleBalance = useCallback(async () => {
    if (!selected) return;
    if (!requireSubscription('Audio Balancer')) return;
    const ext = getExtension(selected.name);
    const result = await run(() => ffmpegEngine.normalizeAudio(selected.file, ext, targetLufs));
    if (result) setOutput(result);
  }, [run, selected, setOutput, targetLufs]);
  ...
  const targetLabel = useMemo(() => `${targetLufs} LUFS`, [targetLufs]);
      ...
      <header className="min-h-14 bg-background border-b border-white/5 flex items-center justify-between px-4 sm:px-6 py-3 shrink-0">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-base sm:text-lg font-semibold truncate">{t('audioBalancer.title')}</h1>
            <p className="text-[11px] sm:text-xs text-zinc-500 truncate">{t('audioBalancer.subtitle')}</p>
          </div>
        </div>
        <div className="hidden sm:flex items-center gap-2 text-xs text-zinc-500">
          <SlidersHorizontal size={14} />
          <span>{t('audioBalancer.badge')}</span>
        </div>
      </header>
            ...
            {!selected && (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    <div className="text-white font-semibold">{t('audioBalancer.uploadTitle')}</div>
                    <div className="text-xs text-zinc-500 mt-1">{t('audioBalancer.uploadDesc')}</div>
                  </div>
                </div>
              </button>
            )}
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-zinc-800/40 border border-white/5 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="text-sm text-zinc-400">{t('audioBalancer.target')}</div>
                    <div className="text-sm text-white font-mono">{targetLabel}</div>
                  </div>
                  <input
                    type="range"
                    min={-20}
                  ...
                  />
                  <div className="text-xs text-zinc-500">{t('audioBalancer.hint')}</div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  >
                    {(state.status === 'loading' || state.status === 'processing') ? (
                      ...
                      <span>{t('audioBalancer.balance')}</span>
                    )}
                  </button>
